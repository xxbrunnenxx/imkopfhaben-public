...
- [x] **Autostart für `brain/`** — bewusst nicht eingerichtet: der Pi 5 wird auch anderweitig genutzt, ein beim Boot automatisch startender Brain-Dienst wäre dort hinderlich. Start bleibt manuell (`brain/imkopfhaben-brain.service` liegt weiterhin unbenutzt im Repo, falls sich das später ändert), siehe README „Installation & Start". **Ergänzt 2026-09-18:** damit der manuelle Start nicht aus mehreren Einzelbefehlen besteht, gibt es jetzt `brain/imkopfhaben-start.sh` (fährt LM Studio :1234 + brain :8000 hoch, wartet bis beide antworten) und `brain/imkopfhaben-stop.sh` (beendet beide, macht den Pi lastfrei). Weiter kein Autostart. Anleitung ohne Vorwissen inkl. Reboot-Fall in `brain/ANLEITUNG.md`. Beide Skripte live getestet (Start, echter `transcribe-raw`-Aufruf mit rohem WAV-Body wie die Firmware → HTTP 200, Stop → beide Ports frei, keine Restprozesse).
